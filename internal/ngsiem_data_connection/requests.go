...
// gofalcon v0.21.0's dataconnectionmanagement spec (EnableHostEnrichment/EnableUserEnrichment
// are plain bool with `,omitempty`, so an explicit false can't be distinguished from unset), not
// a permanent design choice. Best-known hypothesis, to verify (not assume) once CrowdStrike ships
// a fix: those fields become *bool, at which point bodyOverrideParams and these local body structs
// can be deleted in favor of gofalcon's native Create/UpdateDataConnectionRequest types.
//
