...
// TEMPORARY COMPATIBILITY SHIM: this file exists only to work around an unfixed defect in
// gofalcon v0.21.0's dataconnectionmanagement spec (ConnectionTokenResponse.Resources is typed
// []*ConnectionToken though the live API returns a bare object), not a permanent design choice.
// Best-known hypothesis, to verify (not assume) against the regenerated types once CrowdStrike
// ships a fix: Resources becomes a bare/singular object, at which point tokenReader and
// decodeTokenResponse can be deleted in favor of calling ExternalRegenerateDataConnectionToken directly.
//
